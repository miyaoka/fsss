...
| 環境変数の統合                  | ✓    | ✗         | ✗      | ✗                   | ✗       | ✗                     | ✓                 |
| 設定ファイルの統合              | ✓    | ✗         | ✗      | ✗                   | ✗       | ✗                     | ✓                 |
| 1スキーマで CLI/env/config 統合 | ✓    | ✗         | ✗      | ✗                   | ✗       | ✗                     | ✗（独自スキーマ） |
| params/args 分離                | ✓    | ✗         | ✗      | ✗                   | ✓       | ✗                     | —                 |
...
| バリデーション | tskm（1段階）          | 自前                 | commander + Zod（2段階） | yargs-parser のみ      | args-tokens           |
| 遅延ロード     | 逐次 import            | manifest + lazy load | なし（全ロード）         | 逐次 import            | lazy（lazyWithTypes） |
| ランタイム依存 | @tskm/core のみ        | なし                 | React + Ink + commander  | yargs-parser           | args-tokens のみ      |
...
- **Gud CLI** — ファイルベースルーティング + 動的セグメント + params/args 分離を実現しているが、スキーマライブラリ非対応、env/config 統合なし
- **gunshi** — プラグインシステム + 型推論 + マルチランタイム対応。ファイルベースルーティングや env/config 統合はなく、引数定義は args-tokens ベース
- **convict** — CLI/env/config の統合はあるが、独自スキーマ形式でスキーマライブラリ非対応。ファイルベースルーティングなし

**fsss が目指すもの:** Gud CLI のルーティング設計（ファイルベース + 動的セグメント + params/args 分離）と、convict の設定統合思想（1スキーマで CLI/env/config を統一）を、tskm スキーマと bun の上で組み合わせる。どのツールも単独では持っていない「ファイルベースルーティング + スキーマ + CLI/env/config 統合」の3つを1つのフレームワークで提供する。
...
| Zod でリクエストバリデーション | tskm で引数バリデーション      |
